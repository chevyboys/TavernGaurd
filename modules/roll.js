...
let diceEmbed = (titleModifier) => {
    let embed = u.embed({}, true)
        ...
        .setTitle(trimField("Dice Roll " + (titleModifier || ""), 255))
        .setColor("#4682B4")
        .setURL(helpURL)
        .setFooter("For more information and more advanced rolling options, see " + helpURL)
    return embed;
}
...
let rollProcess = (interaction, hiddenResponse = false) => {
    {
        ...
        if (!diceString || diceString == "" || diceString.toLowerCase() == "help") {
            ...
            try {
                ...
                let footerText = trimField(`${diceRoll.output.replace(`${diceRoll.notation}:`, "")}`, 2048);
...
Module.addInteractionCommand({
        ...
        process: async (interaction) => {
            if(interaction.channel == snowflakes.channels.Tavern.main) {
                return interaction.reply({content: "Only the barkeepers get initiative in the Tavern. Try rolling in <#" + snowflakes.channels.Tavern.meta + ">", ephemeral: true})
            }
            else rollProcess(interaction);
        }
