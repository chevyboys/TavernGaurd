...
async function pinCard(message, interaction) {
  try {
    ...
    let content;
    let card = await message.client.channels.cache.get(Module.config.channels.modlogs).send({ content, embeds: [embed], components: (message.author.bot ? undefined : pinActions) });
    return card;
